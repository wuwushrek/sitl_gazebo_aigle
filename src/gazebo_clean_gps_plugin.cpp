#include <gazebo_clean_gps_plugin.h>

namespace gazebo {
GZ_REGISTER_MODEL_PLUGIN(CleanGpsPlugin)

CleanGpsPlugin::CleanGpsPlugin() : ModelPlugin()
{ }

CleanGpsPlugin::~CleanGpsPlugin()
{
  event::Events::DisconnectWorldUpdateBegin(updateConnection_);
}

void CleanGpsPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  updateConnection_ = event::Events::ConnectWorldUpdateBegin(
      boost::bind(&CleanGpsPlugin::OnUpdate, this, _1));

  last_gps_time_ = world_->GetSimTime();
  gravity_W_ = world_->GetPhysicsEngine()->GetGravity();

  gps_pub_ = node_handle_->Advertise<gps_msgs::msgs::SITLGps>("~/" + model_->GetName() + "/gps", 1);
  gt_pub_ = node_handle_->Advertise<gps_msgs::msgs::Groundtruth>("~/" + model_->GetName() + "/groundtruth", 1);
}

void CleanGpsPlugin::OnUpdate(const common::UpdateInfo&){
  ...
  gps_msg.set_altitude(pos_W_I.z + alt_home);
  ...
  gps_msg.set_velocity_up(velocity_current_W.z);

  // add msg to buffer
  //gps_delay_buffer.push(gps_msg);

  // publish SITLGps msg at max rate
...
std::pair<double, double> CleanGpsPlugin::reproject(math::Vector3& pos)
{
